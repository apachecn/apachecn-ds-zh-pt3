...
![$$ \underset{n\to \infty }{\lim}\overline{x}=\mu $$](img/500382_1_En_12_Chapter_TeX_Equd.png)
...
![$$ {s}^2=\frac{\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^2}{n} $$](img/500382_1_En_12_Chapter_TeX_Equh.png)
...
![$$ =\frac{\frac{1}{n}\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^3}{s^3} $$](img/500382_1_En_12_Chapter_TeX_Equp.png)
...
![$$ {G}_1=\frac{\sqrt{n\left(n-1\right)}}{n-2}{g}_1 $$](img/500382_1_En_12_Chapter_TeX_Equr.png)
...
其中![$$ {\overline{x}}_n^1 $$](img/500382_1_En_12_Chapter_TeX_IEq3.png)或简称为![$$ {\overline{x}}_n $$](img/500382_1_En_12_Chapter_TeX_IEq4.png)是平均值或第一个原始样本矩。
...
![$$ =\frac{1}{N-1}\sum \limits_{i=1}^N\left({X}_i-\overline{X}\right)\left({Y}_i-\overline{Y}\right) $$](img/500382_1_En_12_Chapter_TeX_Equae.png)
...
当比率![$$ \frac{p}{n}&lt;1 $$](img/500382_1_En_12_Chapter_TeX_IEq15.png)不可忽略时，样本协方差矩阵是可逆的，但可能在数值上是病态的。(如果一个矩阵的条件数非常大，则称它是病态的。实际上，这样的矩阵几乎是奇异的，并且它的逆的计算，或者线性方程系统的解容易产生大的数值误差。不可逆矩阵的条件数等于无穷大。)这意味着反转它会显著放大估计误差。对于大的 *p* ，很难找到足够多的观测值，大的 *n* ，使![$$ \frac{p}{n} $$](img/500382_1_En_12_Chapter_TeX_IEq16.png)接近于 0，因此开发一个适用于高维协方差矩阵的条件良好的估计器是很重要的。
...
根据手头的情况，对样本协方差矩阵的改进可能是巨大的，也可能是微小的。当![$$ \frac{p}{n} $$](img/500382_1_En_12_Chapter_TeX_IEq17.png)很大和/或群体特征值彼此接近时，线性收缩捕获了对样本协方差矩阵的大部分潜在改进。在相反的情况下，即当![$$ \frac{p}{n} $$](img/500382_1_En_12_Chapter_TeX_IEq18.png)小和/或群体特征值分散时，线性收缩在整个样本协方差矩阵上几乎没有改善。
...
![$$ {F}_X(x)=P\left(X\le x\right) $$](img/500382_1_En_12_Chapter_TeX_Equam.png)

抛一枚公平硬币的 CDF，正面为 0，反面为 1，是一个阶跃函数，如下:

![$$ {F}_X(x)=\left\{\begin{array}{c}0.5,x=0\\ {}1,x=1\end{array}\right. $$](img/500382_1_En_12_Chapter_TeX_Equan.png)

参见图 [12-12](#Fig12) 。

![img/500382_1_En_12_Fig12_HTML.jpg](img/500382_1_En_12_Fig12_HTML.jpg)
...
![img/500382_1_En_12_Fig13_HTML.jpg](img/500382_1_En_12_Fig13_HTML.jpg)
...
![$$ {f}_X(x)=\frac{d}{dx}{F}_X(x) $$](img/500382_1_En_12_Chapter_TeX_Equao.png)T2】
...
![img/500382_1_En_12_Fig14_HTML.jpg](img/500382_1_En_12_Fig14_HTML.jpg)
...
![$$ \mathrm{P}\left(a\le X\le b\right)={\int}_a^bf(x) dx $$](img/500382_1_En_12_Chapter_TeX_Equap.png)

具体来说，我们有这个(见图 [12-15](#Fig15) ):

![$$ {F}_X(x)=\mathrm{P}\left(-\infty &lt;X\le b\right)={\int}_{-\infty}^bf(x) dx $$](img/500382_1_En_12_Chapter_TeX_Equaq.png)

![img/500382_1_En_12_Fig15_HTML.jpg](img/500382_1_En_12_Fig15_HTML.jpg)
...
![$$ {F}_X(x)=\mathrm{P}\left(X\le x\right)=u $$](img/500382_1_En_12_Chapter_TeX_Equar.png)

或者等价地，

![$$ Q(p)=\operatorname{inf}\left\{x\in \mathbb{R}\ |\ u\le F(x)\right\} $$](img/500382_1_En_12_Chapter_TeX_Equas.png)

inf 是给出集合中小于或等于集合中所有元素的最大元素的下确界运算符，也称为最大下界。

一般来说，几乎可以肯定，我们有这样的:

![$$ Q\left(F(X)\right)=X $$](img/500382_1_En_12_Chapter_TeX_Equat.png)
...
![$$ \mathrm{E}(X)=\sum \limits_{-\infty}^{\infty }x\mathrm{P}(x) $$](img/500382_1_En_12_Chapter_TeX_Equau.png)

直观上就是 *X* 的大量独立实现的算术平均值。具有 PDF *f* ( *x* )的连续概率分布的平均值定义如下:

![$$ \mathrm{E}(X)=\mu ={\int}_{-\infty}^{\infty } xf(x) dx $$](img/500382_1_En_12_Chapter_TeX_Equav.png)

均值可能不存在或有限。对于某些概率分布，均值是无穷大(∞或∞),而对于其他概率分布，均值是不确定的。

概率分布的方差定义如下:

![$$ Var(X)={\sigma}^2={\int}_{-\infty}^{\infty }{\left(x-\mu \right)}^2f(x) dx $$](img/500382_1_En_12_Chapter_TeX_Equaw.png)

![$$ ={\int}_{-\infty}^{\infty }{x}^2f(x) dx-{\mu}^2 $$](img/500382_1_En_12_Chapter_TeX_Equax.png)
...
![$$ {\overset{\sim }{\mu}}_k=\frac{\mu_k}{\sigma^k} $$](img/500382_1_En_12_Chapter_TeX_Equay.png)

其中

![$$ {\mu}_k=\mathrm{E}\left[{\left(X-\mu \right)}^k\right]={\int}_{-\infty}^{\infty }{\left(x-\mu \right)}^kf(x) dx $$](img/500382_1_En_12_Chapter_TeX_Equaz.png)

还有

![$$ {\sigma}^k={\left(\sqrt{\mathrm{E}\left[{\left(X-\mu \right)}^2\right]}\right)}^k $$](img/500382_1_En_12_Chapter_TeX_Equba.png)

标准化矩是无量纲的数字。第一个标准化矩是零，因为关于均值的第一个矩总是零。第二个标准化矩是 1，因为关于均值的二阶矩等于方差*σ*T2】2。

概率分布的偏斜度![$$ \gamma ={\overset{\sim }{\mu}}_3 $$](img/500382_1_En_12_Chapter_TeX_IEq19.png)，是第三个标准化矩。这是分布不平衡的一种度量。任何对称分布都有第三个中心矩，如果定义的话，为零。向左侧偏斜的分布(分布的尾部在左侧较长)将具有负偏斜度。向右偏斜的分布(分布的尾部在右边较长)将具有正偏斜度。

概率分布的峰度![$$ \kappa ={\overset{\sim }{\mu}}_4 $$](img/500382_1_En_12_Chapter_TeX_IEq20.png)，是第四个标准化矩。与相同方差的正态分布相比，它是对分布尾部权重的一种度量。因为它是对四次方的期望，第四中心矩，如果定义的话，总是非负的；而且除了一个点分布，它总是严格正的，可以是无界的。如果一个分布有重尾，峰度会很高(称为细峰)；相反，细尾分布(例如，均匀分布等有界分布)的峰度较低(称为平峰度)。
...
![$$ {M}_x(t)=\mathrm{E}\left[{e}^{tX}\right] $$](img/500382_1_En_12_Chapter_TeX_Equbb.png)

即函数是随机变量 *e* <sup>*tX*</sup> 对 *t* ∈ *ℝ* 的一个期望。如果积分不收敛，矩母函数就不存在。

矩母函数如此命名是因为它可以用来计算分布的矩。泰勒展开的*e*<sup>T3】tXT5】给出了如下:</sup>

![$$ {e}^{tX}=1+ tX+\frac{t^2{X}^2}{2!}+\frac{t^3{X}^3}{3!}+\cdots ++\frac{t^n{X}^n}{n!}+\cdots $$](img/500382_1_En_12_Chapter_TeX_Equbc.png)

所以，我们有了这个:

![$$ {M}_x(t)=\mathrm{E}\left[{e}^{tX}\right] $$](img/500382_1_En_12_Chapter_TeX_Equbd.png)

![$$ =1+t\mathrm{E}(X)+\frac{t^2\mathrm{E}\left({X}^2\right)}{2!}+\frac{t^3\mathrm{E}\left({X}^3\right)}{3!}+\cdots ++\frac{t^n\mathrm{E}\left({X}^n\right)}{n!}+\cdots $$](img/500382_1_En_12_Chapter_TeX_Eqube.png)

![$$ =1+t{m}_1+\frac{t^2{m}_2}{2!}+\frac{t^3{m}_3}{3!}+\cdots $$](img/500382_1_En_12_Chapter_TeX_Equbf.png)
...
![$$ {M}_{\alpha X+\beta }(t)=\mathrm{E}\left[{e}^{t\left( aX+\beta \right)}\right]={e}^{\beta t}{M}_X\left(\alpha t\right) $$](img/500382_1_En_12_Chapter_TeX_Equbg.png)

而且，如果 *S* <sub>*n*</sub> 是随机变量的线性组合{ *X* <sub>*i*</sub> }按{*<sub>*I*</sub>}加权，如下图所示:*

*![$$ {S}_n=\sum \limits_{i=1}^n{a}_i{X}_i $$](img/500382_1_En_12_Chapter_TeX_Equbh.png)*

 *那么*S*<sub>T3】nT5】的力矩生成函数如下:</sub>

![$$ {M}_{S_n}(t)={M}_{X_1}\left({a}_1t\right){M}_{X_2}\left({a}_2t\right)\cdots {M}_{X_n}\left({a}_nt\right) $$](img/500382_1_En_12_Chapter_TeX_Equbi.png)
...
数学上，假设{ *X* <sub>1</sub> ， *X* <sub>*n*</sub> }是一个独立同分布(i.i.d .)的随机变量序列，e(*x*<sub>*I*【t15)=*μ*，var(*x*<sub>*I*</sub>)=【t24 然后随着 *n* 趋于无穷大，随机变量![$ \sqrt{n}\left({\overline{X}}_n-\mu \right) $](img/500382_1_En_12_Chapter_TeX_IEq21.png)分布收敛到均值为 0，方差为*σ*T33】2 的正态分布，即 *N* (0，*σ*T39】2)。</sub>

![$$ \sqrt{n}\left({\overline{X}}_n-\mu \right)\to N\left(0,{\sigma}^2\right) $$](img/500382_1_En_12_Chapter_TeX_Equbj.png)

一个正态分布均值 *μ* 和方差 *σ* <sup>2</sup> ，记为 *N* ( *μ* ， *σ* <sup>2</sup> )，具有如下 PDF:

![$$ f(x)=\frac{1}{\sigma \sqrt{2\pi }}\exp \left(-\frac{1}{2}{\left(\frac{x-\mu }{\sigma}\right)}^2\right) $$](img/500382_1_En_12_Chapter_TeX_Equbk.png)

图 [12-16](#Fig16) 显示了一个例子。

![img/500382_1_En_12_Fig16_HTML.jpg](img/500382_1_En_12_Fig16_HTML.jpg)
...
![$$ F(x)=\frac{1}{2}\left[1+\operatorname{erf}\left(\frac{x-\mu }{\sigma \sqrt{2}}\right)\right] $$](img/500382_1_En_12_Chapter_TeX_Equbl.png)

图 [12-17](#Fig17) 显示了一个例子。

![img/500382_1_En_12_Fig17_HTML.jpg](img/500382_1_En_12_Fig17_HTML.jpg)
...
![$$ Q(q)=\mu +\sigma \sqrt{2}{\operatorname{erf}}^{-1}\left(2q-1\right) $$](img/500382_1_En_12_Chapter_TeX_Equbm.png)

意思如下:

![$$ \mathrm{E}(X)=\mu $$](img/500382_1_En_12_Chapter_TeX_Equbn.png)

方差如下:

![$$ Var(X)=\mu $$](img/500382_1_En_12_Chapter_TeX_Equbo.png)

偏斜度如下:

![$$ \gamma =0 $$](img/500382_1_En_12_Chapter_TeX_Equbp.png)

超额峰度如下:

![$$ \kappa =0 $$](img/500382_1_En_12_Chapter_TeX_Equbq.png)

力矩生成函数如下:

![$$ {M}_X(t)=\exp \left(\mu t+\frac{\sigma^2{t}^2}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equbr.png)
...
![$$ {f}_X(x)=\frac{d}{dx}\Pr \left(X\le x\right)=\frac{d}{dx}\Pr \left(\ln X\le \ln x\right) $$](img/500382_1_En_12_Chapter_TeX_Equbs.png)

![$$ =\frac{d}{dx}\Phi \left(\frac{\ln x-\mu }{\sigma}\right) $$](img/500382_1_En_12_Chapter_TeX_Equbt.png)

![$$ =\frac{1}{x\sigma \sqrt{2\pi }}\exp \left(-\frac{{\left(\ln x-\mu \right)}^2}{2{\sigma}^2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equbu.png)

图 [12-18](#Fig18) 显示了一个例子。

![img/500382_1_En_12_Fig18_HTML.jpg](img/500382_1_En_12_Fig18_HTML.jpg)
...
![$$ {F}_X(x)=\Phi \left(\frac{\ln x-\mu }{\sigma}\right)=\frac{1}{2}\operatorname{erfc}\left(-\frac{\ln x-\mu }{\sigma \sqrt{2}}\right) $$](img/500382_1_En_12_Chapter_TeX_Equbv.png)

erfc 是一种称为互补误差函数的特殊函数。

图 [12-19](#Fig19) 显示了一个例子。

![img/500382_1_En_12_Fig19_HTML.jpg](img/500382_1_En_12_Fig19_HTML.jpg)
...
![$$ Q(q)=\exp \left(q+\sigma {Q}_{\Phi}(q)\right) $$](img/500382_1_En_12_Chapter_TeX_Equbw.png)

*Q*<sub>φ</sub>(*Q*)是标准正态分布的分位数函数。

意思如下:

![$$ \mathrm{E}(X)=\exp \left(\mu +\frac{\sigma^2}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equbx.png)

方差如下:

![$$ Var(X)=\left[\exp \left({\sigma}^2\right)-1\right]\exp \left(2\mu +{\sigma}^2\right) $$](img/500382_1_En_12_Chapter_TeX_Equby.png)

偏斜度如下:

![$$ \gamma =\left[\exp \left({\sigma}^2\right)+2\right]\sqrt{\exp \left({\sigma}^2\right)-1} $$](img/500382_1_En_12_Chapter_TeX_Equbz.png)

超额峰度如下:

![$$ \kappa =\exp \left(4{\sigma}^2\right)+2\exp \left(3{\sigma}^2\right)+3\exp \left(2{\sigma}^2\right)-6 $$](img/500382_1_En_12_Chapter_TeX_Equca.png)

力矩生成函数如下:

![$$ {M}_X(t)=\exp \left(\mu t+\frac{\sigma^2{t}^2}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equcb.png)
...
![img/500382_1_En_12_Fig20_HTML.png](img/500382_1_En_12_Fig20_HTML.png)
...
![$$ f\left(x\ |\ \lambda \right)=\left\{\begin{array}{c}\lambda {e}^{-\lambda x},x\ge 0\\ {}0,\kern0.5em x&lt;0\end{array}\right. $$](img/500382_1_En_12_Chapter_TeX_Equcc.png)

图 [12-21](#Fig21) 显示了一个例子。

![img/500382_1_En_12_Fig21_HTML.jpg](img/500382_1_En_12_Fig21_HTML.jpg)
...
![$$ F\left(x\ |\ \lambda \right)=\left\{\begin{array}{c}1-{e}^{-\lambda x},x\ge 0\\ {}0,\kern0.5em x&lt;0\end{array}\right. $$](img/500382_1_En_12_Chapter_TeX_Equcd.png)

图 [12-22](#Fig22) 显示了一个例子。

![img/500382_1_En_12_Fig22_HTML.jpg](img/500382_1_En_12_Fig22_HTML.jpg)
...
![$$ \Pr \left(T&gt;s+t\ |\ T&gt;s\right)=\frac{\Pr \left(T&gt;s+t\cap T&gt;s\right)}{\Pr \left(T&gt;s\right)} $$](img/500382_1_En_12_Chapter_TeX_Equce.png)

![$$ =\frac{\Pr \left(T&gt;s+t\ \right)}{\Pr \left(T&gt;s\right)} $$](img/500382_1_En_12_Chapter_TeX_Equcf.png)

![$$ =\frac{e^{-\lambda \left(s+t\right)}}{e^{-\lambda s}}={e}^{-\lambda t} $$](img/500382_1_En_12_Chapter_TeX_Equcg.png)

![$$ =\Pr \left(T&gt;t\right) $$](img/500382_1_En_12_Chapter_TeX_Equch.png)
...
![$$ Q(q)=-\frac{\ln \left(1-q\right)}{\lambda } $$](img/500382_1_En_12_Chapter_TeX_Equci.png)

意思如下:

![$$ \mathrm{E}(X)=\frac{1}{\lambda } $$](img/500382_1_En_12_Chapter_TeX_Equcj.png)

方差如下:

![$$ Var(X)=\frac{1}{\lambda^2} $$](img/500382_1_En_12_Chapter_TeX_Equck.png)

偏斜度如下:

![$$ \gamma =2 $$](img/500382_1_En_12_Chapter_TeX_Equcl.png)

超额峰度如下:

![$$ \kappa =6 $$](img/500382_1_En_12_Chapter_TeX_Equcm.png)

力矩生成函数如下:

![$$ {M}_X(t)=\frac{\lambda }{\lambda -t},t&lt;\lambda $$](img/500382_1_En_12_Chapter_TeX_Equcn.png)
...
![$$ f\left(k\ |\ \lambda \right)=\Pr \left(X=k\right)=\frac{\lambda^k{e}^{-\lambda }}{k!} $$](img/500382_1_En_12_Chapter_TeX_Equco.png)

其中 *k* = 0，1，2，…是出现的次数。

对于图 [12-23](#Fig23) 中的例子， *k* 是出现的次数。 *λ* 是预期发生率。纵轴是给定 *λ* 的情况下 *k* 发生的概率。该函数仅在 *k* 的整数值上定义；连接线只是眼睛的引导线。

![img/500382_1_En_12_Fig23_HTML.jpg](img/500382_1_En_12_Fig23_HTML.jpg)
...
![$$ F(k)={e}^{-\lambda}\sum \limits_{i=0}^{\underset{\_}{k}}\frac{\lambda^i}{i!} $$](img/500382_1_En_12_Chapter_TeX_Equcp.png)

![$$ \underset{\_}{k} $$](img/500382_1_En_12_Chapter_TeX_IEq22.png)是地板函数，小于等于 *k* 的最小整数。

对于图 [12-23](#Fig23) 中的例子，横轴是指标 *k* ，是出现的次数。CDF 在整数 *k* 处是不连续的，在其他地方是平坦的，因为泊松分布的变量只取整数值。见图 [12-24](#Fig24) 。

![img/500382_1_En_12_Fig24_HTML.jpg](img/500382_1_En_12_Fig24_HTML.jpg)
...
![$$ \mathrm{E}(X)=\lambda $$](img/500382_1_En_12_Chapter_TeX_Equcq.png)

方差如下:

![$$ Var(X)=\lambda $$](img/500382_1_En_12_Chapter_TeX_Equcr.png)

偏斜度如下:

![$$ \gamma ={\lambda}^{-\frac{1}{2}} $$](img/500382_1_En_12_Chapter_TeX_Equcs.png)

超额峰度如下:

![$$ \kappa ={\lambda}^{-1} $$](img/500382_1_En_12_Chapter_TeX_Equct.png)

力矩生成函数如下:

![$$ {M}_X(t)=\exp \left[\lambda \left({e}^t-1\right)\right] $$](img/500382_1_En_12_Chapter_TeX_Equcu.png)
...
而且，如果 *n* 足够大，那么二项随机变量 *X* 可以很好地近似为正态分布。即 *X* ~ *N* ( *np* ，*NP*(1*p*))。等价地，归一化随机变量![$$ z=\frac{x-\mu }{\sigma } $$](img/500382_1_En_12_Chapter_TeX_IEq23.png)具有标准正态分布 *N* (0，1)。

如果随机变量 *X* 服从参数为 *n* ∈ *ℕ* 和*p*∈【0，1】的二项分布，我们写*x*~*b*(*n*， *p* )。在 *n* 次独立伯努利试验中准确获得 *k* 次成功的概率由 PMF 给出:

![$$ f\left(k\ |\ n,p\right)=\Pr \left(k\ |\ n,p\right)=\Pr \left(X=k\right)=\left(\begin{array}{c}n\\ {}k\end{array}\right){p}^k{\left(1-p\right)}^{n-k} $$](img/500382_1_En_12_Chapter_TeX_Equcv.png)

对于 *k* = 0，1，2，…， *n* 。

![$$ \left(\begin{array}{c}n\\ {}k\end{array}\right)=\frac{n!}{k!\left(n-k\right)!} $$](img/500382_1_En_12_Chapter_TeX_IEq24.png)是二项式系数，这是分布得名的原因。公式可以这样理解: *k* 成功发生概率 *p* <sup>* k *</sup> ，*n*—*k*失败发生概率(1—*p*)<sup>*n*—*k*</sup>。然而， *k* 成功可以发生在 *n* 试验中的任何地方，并且在一系列 *k* 试验中有![$$ \left(\begin{array}{c}n\\ {}k\end{array}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq25.png)种不同的分配 *k* 成功的方式。

图 [12-25](#Fig25) 显示了一个例子。

![img/500382_1_En_12_Fig25_HTML.jpg](img/500382_1_En_12_Fig25_HTML.jpg)
...
![$$ F\left(k\ |\ n,p\right)=\Pr \left(X\le k\right)=\sum \limits_{i=0}^{\underset{\_}{k}}\left(\begin{array}{c}n\\ {}i\end{array}\right){p}^i{\left(1-p\right)}^{n-i} $$](img/500382_1_En_12_Chapter_TeX_Equcw.png)

图 [12-26](#Fig26) 显示了一个例子。

![img/500382_1_En_12_Fig26_HTML.jpg](img/500382_1_En_12_Fig26_HTML.jpg)
...
![$$ \mathrm{E}(X)= np $$](img/500382_1_En_12_Chapter_TeX_Equcx.png)

方差如下:

![$$ Var(X)= npq $$](img/500382_1_En_12_Chapter_TeX_Equcy.png)

偏斜度如下:

![$$ \gamma =\frac{q-p}{\sqrt{npq}} $$](img/500382_1_En_12_Chapter_TeX_Equcz.png)

超额峰度如下:

![$$ \kappa =\frac{1-6 pq}{npq} $$](img/500382_1_En_12_Chapter_TeX_Equda.png)

力矩生成函数如下:

![$$ {M}_X(t)={\left(q+p{e}^t\right)}^n $$](img/500382_1_En_12_Chapter_TeX_Equdb.png)
...
![$$ t=\frac{\overline{x}-\mu }{s/\sqrt{n}} $$](img/500382_1_En_12_Chapter_TeX_Equdc.png)

其中![$$ \overline{x} $$](img/500382_1_En_12_Chapter_TeX_IEq26.png)是样本均值，而 *s* 是无偏样本标准差(分母为*n*1)代替真实总体标准差 *σ* 。 *t* 是随机的，因为![$$ \overline{x} $$](img/500382_1_En_12_Chapter_TeX_IEq27.png)和 *s* 是随机的。随机变量 *t* 具有带*υ*=*n*1 自由度的 t 分布。
...
![$$ f(t)=\frac{\Gamma \left(\frac{\upsilon +1}{2}\right)}{\sqrt{\upsilon \pi}\Gamma \left(\frac{\upsilon }{2}\right)}{\left(1+\frac{t^2}{\upsilon}\right)}^{-\frac{\upsilon +1}{2}} $$](img/500382_1_En_12_Chapter_TeX_Equdd.png)

其中 *υ* 是自由度的数量，γ是伽马函数。

图 [12-27](#Fig27) 显示了一个例子。

![img/500382_1_En_12_Fig27_HTML.jpg](img/500382_1_En_12_Fig27_HTML.jpg)
...
![$$ \underset{\upsilon \to \infty }{\lim }t=\frac{\overline{x}-\mu }{\sigma /\sqrt{n}} $$](img/500382_1_En_12_Chapter_TeX_Equde.png)

这是一个均值为 0、方差为 1 的标准正态分布。

CDF 如下:

![$$ F(t)=1-\frac{1}{2}{I}_{x(t)}\left(\frac{\nu }{2},\frac{1}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equdf.png)

其中 *I* 是正则化的不完全 beta 函数，*t*T7】0，我们有这个:

![$$ x(t)=\frac{\nu }{t^2+\nu } $$](img/500382_1_En_12_Chapter_TeX_Equdg.png)

图 [12-28](#Fig28) 显示了一个例子。

![img/500382_1_En_12_Fig28_HTML.jpg](img/500382_1_En_12_Fig28_HTML.jpg)
...
![$$ \upsilon =1, $$](img/500382_1_En_12_Chapter_TeX_Equdh.png)

![$$ Q(q)=\tan \left(\pi \left(q-\frac{1}{2}\right)\right) $$](img/500382_1_En_12_Chapter_TeX_Equdi.png)

![$$ \upsilon =2, $$](img/500382_1_En_12_Chapter_TeX_Equdj.png)

![$$ Q(q)=2\left(q-\frac{1}{2}\right)\sqrt{\frac{2}{\alpha }} $$](img/500382_1_En_12_Chapter_TeX_Equdk.png)

![$$ \alpha =4q\left(1-q\right) $$](img/500382_1_En_12_Chapter_TeX_Equdl.png)

![$$ \upsilon =4, $$](img/500382_1_En_12_Chapter_TeX_Equdm.png)

![$$ Q(q)=\operatorname{sign}\left(q-\frac{1}{2}\right)2\sqrt{u-1} $$](img/500382_1_En_12_Chapter_TeX_Equdn.png)

![$$ u=\frac{\cos \left(\frac{1}{3}{\cos}^{-1}\sqrt{\alpha}\right)}{\sqrt{\alpha }} $$](img/500382_1_En_12_Chapter_TeX_Equdo.png)

sign 函数返回 1 表示正数，-1 表示负数，否则返回 0。

一般来说，分位数函数如下:

![$$ Q(q)=\operatorname{sign}\left({I}^{-1}-\frac{1}{2}\right)\frac{1}{\sqrt{\frac{1}{4\nu {\left({I}^{-1}-\frac{1}{2}\right)}^2}-\frac{1}{\nu }}} $$](img/500382_1_En_12_Chapter_TeX_Equdp.png)

![$$ {I}^{-1}={I}_{\left(\frac{\nu }{2},\frac{\nu }{2}\right)}^{-1}(q) $$](img/500382_1_En_12_Chapter_TeX_IEq28.png)是两个形状参数都为![$$ \frac{\nu }{2} $$](img/500382_1_En_12_Chapter_TeX_IEq29.png)的正则化不完全贝塔函数的逆。

意思如下:

![$$ \mathrm{E}(X)=0 $$](img/500382_1_En_12_Chapter_TeX_Equdq.png)

差异如下:

![$$ Var(X)=\left\{\begin{array}{c}\infty, 1&lt;\nu \le 2\\ {}\frac{\nu }{\nu -2},\nu &gt;2\kern0.5em \end{array}\right. $$](img/500382_1_En_12_Chapter_TeX_IEq30.png)，否则未定义
...
![$$ \kappa =\left\{\begin{array}{c}\infty, 2&lt;\nu \le 4\\ {}\frac{6}{\nu -4},\nu &gt;4\end{array}\right. $$](img/500382_1_En_12_Chapter_TeX_IEq31.png)，否则未定义
...
![$$ Q=\sum \limits_{i=1}^k{Z}_i^2 $$](img/500382_1_En_12_Chapter_TeX_Equdr.png)

具有卡方分布 *k* 自由度。这通常表示为*Q*~*χ*<sup>2</sup>(*k*)或![$$ Q\sim {\chi}_k^2 $$](img/500382_1_En_12_Chapter_TeX_IEq32.png)。卡方分布有一个参数:一个正整数 *k* 指定自由度(被求和的随机变量的数量，*Z*<sub>*I*</sub>s)。

此外，样本方差 *S* <sup>2</sup> 具有标度卡方分布。![$$ {S}^2=\frac{1}{n-1}\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^2 $$](img/500382_1_En_12_Chapter_TeX_Equds.png)

![$$ {S}^2\sim \frac{\sigma^2}{n-1}{\chi}_{n-1}^2 $$](img/500382_1_En_12_Chapter_TeX_Equdt.png)

等价地，我们有这样的:

![$$ \frac{S^2}{\sigma^2}\sim \frac{\chi_{n-1}^2}{n-1} $$](img/500382_1_En_12_Chapter_TeX_Equdu.png)

卡方分布的 PDF 如下:

![$$ f\left(x\ |\ k\right)=\frac{x^{\frac{k}{2}-1}{e}^{-\frac{x}{2}}}{2^{\frac{k}{2}}\Gamma \left(\frac{k}{2}\right)},x&gt;0 $$](img/500382_1_En_12_Chapter_TeX_Equdv.png)

其中![$$ \Gamma \left(\frac{k}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq33.png)是伽马函数。

图 [12-29](#Fig29) 显示了一个例子。

![img/500382_1_En_12_Fig29_HTML.png](img/500382_1_En_12_Fig29_HTML.png)
...
![$$ F\left(x\ |\ k\right)=\frac{\gamma \left(\frac{k}{2},\frac{x}{2}\right)}{\Gamma \left(\frac{k}{2}\right)}=P\left(\frac{k}{2},\frac{x}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equdw.png)

其中 *γ* ( *s* ， *t* )为下不完全伽马函数， *P* ( *s* ， *t* )为正则化不完全伽马函数。

图 [12-30](#Fig30) 显示了一个例子。

![img/500382_1_En_12_Fig30_HTML.png](img/500382_1_En_12_Fig30_HTML.png)
...
![$$ Q(q)={P}^{-1}\left(\frac{k}{2},\frac{q}{2}\right)=2{P}^{-1}\left(\frac{k}{2},q\right) $$](img/500382_1_En_12_Chapter_TeX_Equdx.png)T2】

意思如下:

![$$ \mathrm{E}(X)=k $$](img/500382_1_En_12_Chapter_TeX_Equdy.png)

方差如下:

![$$ Var(X)=2k $$](img/500382_1_En_12_Chapter_TeX_Equdz.png)

偏斜度如下:

![$$ \gamma =\sqrt{\frac{8}{k}} $$](img/500382_1_En_12_Chapter_TeX_Equea.png)

超额峰度如下:

![$$ \kappa =\frac{12}{k} $$](img/500382_1_En_12_Chapter_TeX_Equeb.png)

力矩生成函数如下:

![$$ {M}_X(t)={\left(1-2t\right)}^{-\frac{k}{2}},t&lt;\frac{1}{2} $$](img/500382_1_En_12_Chapter_TeX_Equec.png)
...
*![$$ \frac{\chi_{d_1}^2}{\chi_{d_2}^2}\sim F\left({d}_1,{d}_2\right) $$](img/500382_1_En_12_Chapter_TeX_Equed.png)*

而且，样本方差 *S* <sup>2</sup> 具有卡方分布。

![$$ {S}^2=\frac{1}{n-1}\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^2\sim \frac{\sigma^2}{n-1}{\chi}_{n-1}^2 $$](img/500382_1_En_12_Chapter_TeX_Equee.png)

那么两个样本的样本方差比![$$ \frac{S_1^2}{S_2^2} $$](img/500382_1_En_12_Chapter_TeX_IEq34.png)和缩放样本方差比![$$ \frac{S_1^2/{\sigma}_1^2}{S_2^2/{\sigma}_2^2} $$](img/500382_1_En_12_Chapter_TeX_IEq35.png)遵循 f 分布。而且对于自由度为 *ν* ，*t*T8】2 的随机变量 *t* 具有 f 分布 *F* (1， *ν* )。

如果一个随机变量 *X* 有一个参数为 *d* <sub>1</sub> > 0 和 *d* <sub>2</sub> > 0 的 f 分布，我们写*X*~*F*(*d*<sub>1</sub>， *d* <sub>2</sub> )。PDF 如下:

![$$ f\left(x\ |\ {d}_1,{d}_2\right)=\frac{\sqrt{\frac{{\left({d}_1x\right)}^{d_1}{d_2}^{d_2}}{{\left({d}_1x+{d}_2\right)}^{d_1+{d}_2}}}}{xB\left(\frac{d_1}{2},\frac{d_2}{2}\right)} $$](img/500382_1_En_12_Chapter_TeX_Equef.png)T24】

*B* 是贝塔函数。

图 [12-31](#Fig31) 显示了一个例子。

![img/500382_1_En_12_Fig31_HTML.png](img/500382_1_En_12_Fig31_HTML.png)
...
![$$ F\left(x\ |\ {d}_1,{d}_2\right)={I}_{\frac{d_1x}{d_1x+{d}_2}}\left(\frac{d_1}{2},\frac{d_2}{2}\right) $$](img/500382_1_En_12_Chapter_TeX_Equeg.png)

*I* 是正则化的不完全 beta 函数。

图 [12-32](#Fig32) 显示了一个例子。

![img/500382_1_En_12_Fig32_HTML.png](img/500382_1_En_12_Fig32_HTML.png)
...
![$$ Q(q)=\frac{d_2{I}^{-1}}{d_1-{d}_1{I}^{-1}} $$](img/500382_1_En_12_Chapter_TeX_Equeh.png)

![$$ {I}^{-1}={I}_{\left(\frac{d_1}{2},\frac{d_2}{2}\right)}^{-1}(q) $$](img/500382_1_En_12_Chapter_TeX_IEq36.png)是具有形状参数![$$ \frac{d_1}{2} $$](img/500382_1_En_12_Chapter_TeX_IEq37.png)和![$$ \frac{d_2}{2} $$](img/500382_1_En_12_Chapter_TeX_IEq38.png)的正则化不完全贝塔函数的逆。

意思如下:

![$$ \mathrm{E}(X)=\frac{d_2}{d_2-2},{d}_2&gt;2 $$](img/500382_1_En_12_Chapter_TeX_Equei.png)

方差如下:

![$$ Var(X)=\frac{2{d}_2^2\left({d}_1+{d}_2-2\right)}{d_1{\left({d}_2-2\right)}^2\left({d}_2-4\right)},{d}_2&gt;4 $$](img/500382_1_En_12_Chapter_TeX_Equej.png)

偏斜度如下:

![$$ \gamma =\frac{\left(2{d}_1+{d}_2-2\right)\sqrt{8\left({d}_2-4\right)}}{\left({d}_2-6\right)\sqrt{d_1\left({d}_1+{d}_2-2\right)}},{d}_2&gt;6 $$](img/500382_1_En_12_Chapter_TeX_Equek.png)

超额峰度如下:

![$$ \kappa =12\frac{d_1\left(5{d}_2-22\right)\left({d}_1+{d}_2-2\right)+\left({d}_2-4\right){\left({d}_2-2\right)}^2}{d_1\left({d}_2-6\right)\left({d}_2-8\right)\left({d}_1+{d}_2-2\right)} $$](img/500382_1_En_12_Chapter_TeX_Equel.png)
...
瑞利分布是非负值随机变量的连续概率分布。它本质上是具有两个自由度的卡方分布。例如，![$$ X=\sqrt{X_1^2+{X}_2^2} $$](img/500382_1_En_12_Chapter_TeX_IEq39.png)遵循瑞利分布，其中*X*T3】1 和*X*T7】2 是两个正态分布，![$$ {X}_1^2 $$](img/500382_1_En_12_Chapter_TeX_IEq40.png)和![$$ {X}_2^2 $$](img/500382_1_En_12_Chapter_TeX_IEq41.png)是两个卡方分布。
...
另一种可能是![$$ X=\sigma \sqrt{-2\ln U} $$](img/500382_1_En_12_Chapter_TeX_IEq42.png)，其中 *U* 是均匀分布的随机变量，具有参数 *σ* 的瑞利分布。

PDF 如下:

![$$ f\left(x\ |\ \sigma \right)=\frac{x}{\sigma^2}\exp \left(-\frac{x^2}{2{\sigma}^2}\right),x\ge 0 $$](img/500382_1_En_12_Chapter_TeX_Equem.png)

其中 *σ* 是分布的比例参数。

图 [12-33](#Fig33) 显示了一个例子。

![img/500382_1_En_12_Fig33_HTML.jpg](img/500382_1_En_12_Fig33_HTML.jpg)
...
![$$ F\left(x\ |\ \sigma \right)=1-\exp \left(-\frac{x^2}{2{\sigma}^2}\right),x\ge 0 $$](img/500382_1_En_12_Chapter_TeX_Equen.png)

图 [12-34](#Fig34) 显示了一个例子。

![img/500382_1_En_12_Fig34_HTML.jpg](img/500382_1_En_12_Fig34_HTML.jpg)
...
![$$ Q(q)=\sigma \sqrt{-2\ln \left(1-q\right)} $$](img/500382_1_En_12_Chapter_TeX_Equeo.png)

意思如下:

![$$ \mathrm{E}(X)=\sigma \sqrt{\frac{\pi }{2}} $$](img/500382_1_En_12_Chapter_TeX_Equep.png)

方差如下:

![$$ Var(X)=\frac{4-\pi }{2}{\sigma}^2 $$](img/500382_1_En_12_Chapter_TeX_Equeq.png)

偏斜度如下:

![$$ \gamma =\frac{2\sqrt{\pi}\left(\pi -3\right)}{{\left(4-\pi \right)}^{\frac{3}{2}}} $$](img/500382_1_En_12_Chapter_TeX_Equer.png)

超额峰度如下:

![$$ \kappa =-\frac{6{\pi}^2-24\pi +16}{{\left(4-\pi \right)}^2} $$](img/500382_1_En_12_Chapter_TeX_Eques.png)

力矩生成函数如下:

![$$ {M}_X(t)=1+\sigma t\exp \left(\frac{\sigma^2{t}^2}{2}\right)\sqrt{\frac{\pi }{2}}\left(\operatorname{erf}\left(\frac{\sigma t}{\sqrt{2}}\right)+1\right) $$](img/500382_1_En_12_Chapter_TeX_Equet.png)
...
*   具有形状参数*α*=*k*T5】0 和称为速率参数的逆比例参数![$$ \beta =\frac{1}{\theta }&gt;0 $$](img/500382_1_En_12_Chapter_TeX_IEq43.png)
...
![$$ X\sim \Gamma \left(k,\theta \right) $$](img/500382_1_En_12_Chapter_TeX_Equeu.png)

PDF 如下:

![$$ f\left(x\ |\ k,\theta \right)=\frac{x^{k-1}\exp \left(-\frac{x}{\theta}\right)}{\theta^k\Gamma (k)} $$](img/500382_1_En_12_Chapter_TeX_Equev.png)

其中γ(*k*)是在 *k* 处计算的伽马函数。

图 [12-35](#Fig35) 显示了一个例子。

![img/500382_1_En_12_Fig35_HTML.png](img/500382_1_En_12_Fig35_HTML.png)
...
![$$ f\left(x\ |\ k,\theta \right)=\frac{\gamma \left(k,\frac{x}{\theta}\right)}{\Gamma (k)} $$](img/500382_1_En_12_Chapter_TeX_Equew.png)

图 [12-36](#Fig36) 显示了一个例子。

![img/500382_1_En_12_Fig36_HTML.png](img/500382_1_En_12_Fig36_HTML.png)
...
![$$ Q(q)=\theta {P}^{-1}\left(k,q\right) $$](img/500382_1_En_12_Chapter_TeX_Equex.png)

其中*P*T2 1 是正则化不完全伽马函数的逆。

意思如下:

![$$ \mathrm{E}(X)= k\theta $$](img/500382_1_En_12_Chapter_TeX_Equey.png)

方差如下:

![$$ Var(X)=k{\theta}^2 $$](img/500382_1_En_12_Chapter_TeX_Equez.png)

偏斜度如下:

![$$ \gamma =\frac{2}{k} $$](img/500382_1_En_12_Chapter_TeX_Equfa.png)

超额峰度如下:

![$$ \kappa =\frac{6}{k} $$](img/500382_1_En_12_Chapter_TeX_Equfb.png)

力矩生成函数如下:

![$$ {M}_X(t)={\left(1-\theta t\right)}^{-k},t&lt;\frac{1}{\theta } $$](img/500382_1_En_12_Chapter_TeX_Equfc.png)
...
![$$ X\sim \mathrm{Beta}\left(\alpha, \beta \right) $$](img/500382_1_En_12_Chapter_TeX_Equfd.png)

PDF 如下:

![$$ f\left(x\ |\ \alpha, \beta \right)=\frac{1}{\mathrm{B}\left(\alpha, \beta \right)}{x}^{\alpha -1}{\left(1-x\right)}^{\beta -1} $$](img/500382_1_En_12_Chapter_TeX_Equfe.png)

其中 B( *α* ， *β* )为贝塔函数。

图 [12-37](#Fig37) 显示了一个例子。

![img/500382_1_En_12_Fig37_HTML.jpg](img/500382_1_En_12_Fig37_HTML.jpg)
...
![$$ f\left(x\ |\ \alpha, \beta \right)=\frac{\mathrm{B}\left(x\ |\ \alpha, \beta \right)}{\mathrm{B}\left(\alpha, \beta \right)}={I}_x\left(\alpha, \beta \right) $$](img/500382_1_En_12_Chapter_TeX_Equff.png)

其中 B( *x* | *α* ， *β* )为不完全 beta 函数，*I*<sub>*x*</sub>(*α*， *β* )为正则化不完全 beta 函数。

图 [12-38](#Fig38) 显示了一个例子。

![img/500382_1_En_12_Fig38_HTML.jpg](img/500382_1_En_12_Fig38_HTML.jpg)
...
![$$ Q(q)={I}_q^{-1}\left(\alpha, \beta \right) $$](img/500382_1_En_12_Chapter_TeX_Equfg.png)T2】

意思如下:

![$$ \mathrm{E}(X)=\frac{\alpha }{\alpha +\beta } $$](img/500382_1_En_12_Chapter_TeX_Equfh.png)

方差如下:

![$$ Var(X)=\frac{\alpha \beta}{{\left(\alpha +\beta \right)}^2\left(\alpha +\beta +1\right)} $$](img/500382_1_En_12_Chapter_TeX_Equfi.png)

偏斜度如下:

![$$ \gamma =\frac{2\left(\beta -\alpha \right)\sqrt{\alpha +\beta +1}}{\left(\alpha +\beta +2\right)\sqrt{\alpha \beta}} $$](img/500382_1_En_12_Chapter_TeX_Equfj.png)

超额峰度如下:

![$$ \kappa =\frac{6\left[{\left(\alpha -\beta \right)}^2\left(\alpha +\beta +1\right)-\alpha \beta \left(\alpha +\beta +2\right)\right]}{\alpha \beta \left(\alpha +\beta +1\right)\left(\alpha +\beta +3\right)} $$](img/500382_1_En_12_Chapter_TeX_Equfk.png)

力矩生成函数如下:

![$$ {M}_X(t)=1+\sum \limits_{k=1}^{\infty}\left(\prod \limits_{r=0}^{k-1}\frac{\alpha +r}{a+\beta +r}\right)\frac{t^k}{k!} $$](img/500382_1_En_12_Chapter_TeX_Equfl.png)
...
![$$ f\left(x\ |\ \lambda, k\right)=\frac{k}{\lambda }{\left(\frac{x}{\lambda}\right)}^{k-1}\exp \left(-{\left(\frac{x}{\lambda}\right)}^k\right) $$](img/500382_1_En_12_Chapter_TeX_Equfm.png)

图 [12-39](#Fig39) 显示了一个例子。

![img/500382_1_En_12_Fig39_HTML.jpg](img/500382_1_En_12_Fig39_HTML.jpg)
...
![$$ F\left(x\ |\ \lambda, k\right)=1-\exp \left(-{\left(\frac{x}{\lambda}\right)}^k\right) $$](img/500382_1_En_12_Chapter_TeX_Equfn.png)

图 [12-40](#Fig40) 显示了一个例子。

![img/500382_1_En_12_Fig40_HTML.jpg](img/500382_1_En_12_Fig40_HTML.jpg)
...
![$$ Q(q)=\lambda {\left(-\ln \left(1-q\right)\right)}^{\frac{1}{k}} $$](img/500382_1_En_12_Chapter_TeX_Equfo.png)

意思如下:

![$$ \mathrm{E}(X)=\lambda \Gamma \left(1+\frac{1}{k}\right) $$](img/500382_1_En_12_Chapter_TeX_Equfp.png)

γ是伽玛函数。

方差如下:

![$$ Var(X)={\lambda}^2\left[\Gamma \left(1+\frac{1}{2}\right)-{\left(\Gamma \left(1+\frac{1}{k}\right)\right)}^2\right] $$](img/500382_1_En_12_Chapter_TeX_Equfq.png)

偏斜度如下:

![$$ \gamma =\frac{\lambda^3\Gamma \left(1+\frac{3}{k}\right)-3\mu {\sigma}^2-{\mu}^3}{\sigma^3} $$](img/500382_1_En_12_Chapter_TeX_Equfr.png)

超额峰度如下:

![$$ \kappa =\frac{\lambda^4\Gamma \left(1+\frac{4}{k}\right)-4\gamma {\sigma}^3\mu -6{\mu}^2{\sigma}^2-{\mu}^4}{\sigma^4}-3 $$](img/500382_1_En_12_Chapter_TeX_Equfs.png)

力矩生成函数如下:

![$$ {M}_X(t)=\sum \limits_{n=0}^{\infty}\frac{t^n{\lambda}^n}{n!}\Gamma \left(1+\frac{n}{k}\right),k\ge 1 $$](img/500382_1_En_12_Chapter_TeX_Equft.png)
...
具体来说，经验累积分布函数是在每个 *n* 数据点上跳跃![$$ \frac{1}{n} $$](img/500382_1_En_12_Chapter_TeX_IEq44.png)的阶跃函数。它在随机变量的任何指定值处的值是小于或等于指定值的变量观察值的分数。经验分布函数是对产生样本点的累积分布函数的估计。根据 Glivenko-Cantelli 定理，它以概率 1 收敛于该基本分布。

数学上，设{*X*T2】1，...、 *X* <sub>*n*</sub> }为独立同分布的实随机变量，具有相同的累积分布函数 *F* ( *x* )。那么经验累积分布函数定义如下:

![$$ {\hat{F}}_n(x)=\frac{1}{n}\sum \limits_{i=1}^n{\mathbf{1}}_{X_i\le x} $$](img/500382_1_En_12_Chapter_TeX_Equfu.png)

其中**1**<sub>T3A</sub>是事件 *A* 的指示器。当条件或事件满足时，它等于 1。这种情况下，条件是*X*<sub>*I*</sub>≤*X*。![$$ {\hat{F}}_n(x) $$](img/500382_1_En_12_Chapter_TeX_IEq45.png)是 *F* ( *x* )的无偏估计量。
...
![img/500382_1_En_12_Fig41_HTML.jpg](img/500382_1_En_12_Fig41_HTML.jpg)

图 12-41

经验分布与正态分布

样本量 *n* 的经验分布的分位数函数是这样的，如果 *nq* 不是整数，那么第 *q* 个分位数是唯一的，并且等于第![$$ \overline{nq} $$](img/500382_1_En_12_Chapter_TeX_IEq46.png)个样本，其中![$$ \overline{nq} $$](img/500382_1_En_12_Chapter_TeX_IEq47.png)是小于或等于 *nq* 的最大整数。

![$$ Q(q)={x}_{\overline{nq}} $$](img/500382_1_En_12_Chapter_TeX_Equfv.png)

如果 *nq* 是整数，那么 *q* 的分位数不是唯一的，而是任意实数 *x* ，使得:

![$$ {x}_{nq}&lt;x&lt;{x}_{nq+1} $$](img/500382_1_En_12_Chapter_TeX_Equfw.png)

均值是总体分布均值的无偏估计量:

![$$ {\mathrm{E}}_n(X)=\overline{x}=\frac{1}{n}\left(\sum \limits_{i=1}^n{x}_i\right) $$](img/500382_1_En_12_Chapter_TeX_Equfx.png)

方差如下:

![$$ Var(X)=\frac{1}{n}\left(\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^2\right) $$](img/500382_1_En_12_Chapter_TeX_Equfy.png)
...
![$$ {F}_{X,Y}\left(x,y\right)=\mathrm{P}\left(X\le x,Y\le y\right) $$](img/500382_1_En_12_Chapter_TeX_Equga.png)

右边表示随机变量 *X* 取值小于或等于 *x* 以及 *Y* 取值小于或等于 *y* 的概率。

连续随机变量的联合概率密度函数定义为联合 CDF 对所有变量各一次的偏导数，如下:

![$$ {f}_{\boldsymbol{X}}\left(\boldsymbol{x}\right)=\frac{\partial^n{F}_{\boldsymbol{X}}\left(\boldsymbol{x}\right)}{\partial {x}_1\dots \partial {x}_n} $$](img/500382_1_En_12_Chapter_TeX_Equgb.png)

在两个随机变量 *X* 和 *Y* 的情况下，联合 PDF 简化为:

![$$ {f}_{X,Y}\left(x,y\right)=\frac{\partial^2{F}_{X,Y}\left(x,y\right)}{\partial x\partial y} $$](img/500382_1_En_12_Chapter_TeX_Equgc.png)

这等于下面:

![$$ {f}_{X,Y}\left(x,y\right)={f}_{Y\mid X}\left(y\ |\ x\right){f}_X(x)={f}_{X\mid Y}\left(x\ |\ y\right){f}_Y(y) $$](img/500382_1_En_12_Chapter_TeX_Equgd.png)
...
![img/500382_1_En_12_Fig42_HTML.png](img/500382_1_En_12_Fig42_HTML.png)
...
**![$$ {f}_{\boldsymbol{X}}\left(\boldsymbol{x}\right)=\frac{1}{\sqrt{{\left(2\pi \right)}^k\left|\boldsymbol{\Sigma} \right|}}\exp \left(-\frac{1}{2}{\left(\boldsymbol{x}-\boldsymbol{\mu} \right)}^T{\boldsymbol{\Sigma}}^{-1}\left(\boldsymbol{x}-\boldsymbol{\mu} \right)\right) $$](img/500382_1_En_12_Chapter_TeX_Equgh.png)**

 **如果**σ**是 1 × 1 矩阵(即单个实数)，则之前的等式简化为单变量正态分布的等式。

在两个随机变量 *X* 和 *Y* 的情况下，一个向量[*XY*<sup>*T*</sup>的联合 PDF 如下:

![$$ f\left(x,y\right)=\frac{1}{2\pi {\sigma}_X{\sigma}_Y\sqrt{1-{\rho}^2}}\exp \left(-\frac{1}{2\left(1-{\rho}^2\right)}\left[{\left(\frac{x-{\mu}_X}{\sigma_X}\right)}^2-2\rho \left(\frac{x-{\mu}_X}{\sigma_X}\right)\left(\frac{y-{\mu}_Y}{\sigma_Y}\right)+{\left(\frac{y-{\mu}_Y}{\sigma_Y}\right)}^2\right]\right) $$](img/500382_1_En_12_Chapter_TeX_Equgi.png)

其中 *ρ* 为 *X* 与 *Y* 的相关性，其中*σ*<sub>*X*</sub>>0 与*σ*<sub>*Y*</sub>>0。就更一般的多元正态 PDF 而言，我们有:

![$$ \boldsymbol{\mu} =\left(\begin{array}{c}{\mu}_X\\ {}{\mu}_Y\end{array}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq48.png)和![$$ \boldsymbol{\Sigma} =\left(\begin{array}{cc}{\sigma}_X^2&amp; \rho {\sigma}_X{\sigma}_Y\\ {}\rho {\sigma}_X{\sigma}_Y&amp; {\sigma}_Y^2\end{array}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq49.png)

例如，图 [12-43](#Fig43) 显示了二元正态分布的样本点，如下所示:

![img/500382_1_En_12_Fig43_HTML.png](img/500382_1_En_12_Fig43_HTML.png)

图 12-43

二元正态分布

![$$ \boldsymbol{\mu} =\left(\begin{array}{c}0\\ {}0\end{array}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq50.png)和![$$ \boldsymbol{\Sigma} =\left(\begin{array}{cc}1&amp; 0.6\\ {}0.6&amp; 1\end{array}\right) $$](img/500382_1_En_12_Chapter_TeX_IEq51.png)

还显示了两个边际分布、一维直方图和三西格马椭圆。

意思如下:

![$$ \mathrm{E}\left(\boldsymbol{X}\right)=\boldsymbol{\mu} $$](img/500382_1_En_12_Chapter_TeX_Equgj.png)

方差如下:

![$$ Var\left(\boldsymbol{X}\right)=\boldsymbol{\Sigma} $$](img/500382_1_En_12_Chapter_TeX_Equgk.png)

力矩生成函数如下:

![$$ {M}_X(t)=\exp \left({\boldsymbol{\mu}}^T\boldsymbol{t}+\frac{1}{2}{\boldsymbol{t}}^T\boldsymbol{\Sigma} \mathbf{t}\right) $$](img/500382_1_En_12_Chapter_TeX_Equgl.png)
...
![$$ f\left({x}_1,\dots |{x}_K,\ {a}_1,\dots, {a}_K\right)=\frac{1}{B\left(\boldsymbol{a}\right)}\prod \limits_{i=1}^K{x_i}^{a_i-1} $$](img/500382_1_En_12_Chapter_TeX_Equgn.png)

其中![$$ \sum \limits_{i=1}^K{x}_i=1 $$](img/500382_1_En_12_Chapter_TeX_IEq55.png)和 *x* <sub> * i * </sub> ≥ 0 为所有 *i* 。

归一化常数是多元 beta 函数，可以用 gamma 函数来表示，如下:

![$$ B\left(\boldsymbol{a}\right)=\frac{\prod \limits_{i=1}^K\Gamma \left({a}_i\right)}{\Gamma \left(\sum \limits_{i=1}^K{a}_i\right)} $$](img/500382_1_En_12_Chapter_TeX_Equgo.png)

图 [12-44](#Fig44) 显示了一个例子。

![img/500382_1_En_12_Fig44_HTML.jpg](img/500382_1_En_12_Fig44_HTML.jpg)
...
数学上，我们有 *k* 个可能互斥的结果，对应概率 *p* <sub>1</sub> ，…， *p* <sub>*k*</sub> 和 *n* 个独立试验。由于 *k* 的结果是互斥的，并且其中一个必须发生，所以对于 *i* = 1，…， *k* 和![$$ \sum \limits_{i=1}^k{p}_i=1 $$](img/500382_1_En_12_Chapter_TeX_IEq56.png)，我们有*p*<sub>*I*</sub>≥0。

如果随机变量{ *X* <sub>*i*</sub> }表示在 *n* 次试验中观察到第 *i* 次结果的次数，则随机向量*X*=(*X*<sub>1</sub>，…，*X*<sub>*k*</sub>)<sup>*T*虽然试验是独立的，但是它们的结果 *X* 是相关的，因为它们必须被加起来 *n* ，![$ \sum \limits_{i=1}^k{x}_i=n $](img/500382_1_En_12_Chapter_TeX_IEq57.png)。</sup>

多项式分布是一种离散的概率分布。概率质量函数描述了同时发生的 *k* 个事件的联合概率。【T2![$$ {p}_{X_1,\dots, {X}_k}\left({x}_1,\dots, {x}_k\right)=\mathrm{P}\left({X}_1={x}_1\wedge \dots \wedge {X}_k={x}_k\right) $$](img/500382_1_En_12_Chapter_TeX_Equgp.png)

所有结果的总和等于 1。

![$$ \sum \limits_i\sum \limits_j\dots \sum \limits_n\mathrm{P}\left({X}_1={x}_{1i},\dots, {X}_k={x}_{kn}\right)=1 $$](img/500382_1_En_12_Chapter_TeX_Equgq.png)T2】

这种多项分布的概率质量函数给出了从一个袋子中挑选 *k* 种不同颜色的 *n* 个球的概率，在每次抽取后替换被抽取的球。相同颜色的球是等价的。![$$ f\left({x}_1,\dots |{x}_k,\ n,{p}_1,\dots, {p}_k\right)=\mathrm{P}\left({X}_1={x}_1,\dots, {X}_k={x}_k\right) $$](img/500382_1_En_12_Chapter_TeX_Equgr.png)

![$$ =\frac{n!}{x_1!\dots {x}_k!}{p}_1^{x_1}\times \dots \times {p}_k^{x_k} $$](img/500382_1_En_12_Chapter_TeX_Equgs.png)

意思如下:

![$$ \mathrm{E}\left({X}_i\right)=n{p}_i $$](img/500382_1_En_12_Chapter_TeX_Equgt.png)

方差和协方差如下:

![$$ Var\left({X}_i\right)=n{p}_i\left(1-{p}_i\right) $$](img/500382_1_En_12_Chapter_TeX_Equgu.png)

![$$ Cov\left({X}_i,{X}_j\right)=-n{p}_i{p}_j,i\ne j $$](img/500382_1_En_12_Chapter_TeX_Equgv.png)

力矩生成函数如下:

![$$ {M}_X(t)={\left(\sum \limits_{i=1}^k{p}_i{e}^{t_i}\right)}^n $$](img/500382_1_En_12_Chapter_TeX_Equgw.png)

假设我们将 100 个球扔进 3 个箱子，每个箱子的成功概率分别为 0.1、0.2 和 0.7。每个箱中有 10、20 和 70 的概率是 0.013279。见图 [12-45](#Fig45) 。

![img/500382_1_En_12_Fig45_HTML.png](img/500382_1_En_12_Fig45_HTML.png)
...
再举一个例子，假设我们怀疑一个硬币是有偏向的，在旋转的时候，正面着地的机会少于 50%。为了验证这个猜想，我们做了一个实验，将硬币旋转 100 次，得到 37 个正面和 63 个反面。观察到的人头比例为![$$ \frac{37}{100}=37\ \mathrm{percent} $$](img/500382_1_En_12_Chapter_TeX_IEq58.png)。我们对这些数据有两种可能的解释。
...
![$$ z=\frac{0.37-0.5}{0.05}=-2.6 $$](img/500382_1_En_12_Chapter_TeX_Equgx.png)
...
![img/500382_1_En_12_Fig46_HTML.jpg](img/500382_1_En_12_Fig46_HTML.jpg)
...
![img/500382_1_En_12_Fig47_HTML.png](img/500382_1_En_12_Fig47_HTML.png)
...
5.  根据观察值计算检验统计量 *T* 的观察值![$$ \hat{t} $$](img/500382_1_En_12_Chapter_TeX_IEq59.png)。计算 p 值。这是在零假设下，抽样检验统计量至少与观察到的一样极端的概率(该事件的最大概率)。
...
![$$ W=\frac{{\left(\sum \limits_{i=i}^n{a}_i{x}_{(i)}\right)}^2}{\sum \limits_{i=1}^n{\left({x}_i-\overline{x}\right)}^2} $$](img/500382_1_En_12_Chapter_TeX_Equgy.png)

其中 *x* <sub>( *i* )</sub> (用括号括起下标索引*I*；不要和 *x* <sub>*i*</sub> 混淆)是第 *i* 阶统计量。为了找到订单统计数据，我们需要对样本进行排序，并找到样本中第 *i* 个最小的数字。![$$ \overline{x} $$](img/500382_1_En_12_Chapter_TeX_IEq60.png)是样本均值。

系数 *a* <sub>*i*</sub> 给出如下:

![$$ \left({a}_1,\dots, {a}_n\right)=\frac{m^T{V}^{-1}}{C} $$](img/500382_1_En_12_Chapter_TeX_Equgz.png)

其中 *C* 为向量范数，如下图:

![$$ C=\left\Vert {V}^{-1}m\right\Vert ={\left({m}^T{V}^{-1}{V}^{-1}m\right)}^{\frac{1}{2}} $$](img/500382_1_En_12_Chapter_TeX_Equha.png)
...
![$$ JB=\frac{n}{6}\left({S}^2+\frac{1}{4}{\left(K-3\right)}^2\right) $$](img/500382_1_En_12_Chapter_TeX_Equhb.png)
...
在检验正态性方面，达戈斯蒂诺检验比 Jarque-Bera 检验更有效。这也是偏离常态的拟合优度度量。该测试基于样本峰度和偏斜度的变换。它结合了偏度测试和峰度测试。偏斜度测试确定数据的偏斜度在统计上是否不同于零。测试统计是![$$ {z}_s^2 $$](img/500382_1_En_12_Chapter_TeX_IEq61.png)。同样，峰度测试确定数据的过度峰度在统计上是否不为零。测试统计是![$$ {z}_k^2 $$](img/500382_1_En_12_Chapter_TeX_IEq62.png)。达戈斯蒂诺检验是基于这样一个事实:当数据呈正态分布时，检验统计量![$$ T={z}_s^2+{z}_k^2 $$](img/500382_1_En_12_Chapter_TeX_IEq63.png)具有两个自由度的卡方分布。这个测试通常不应该用于少于 20 个元素的数据集。零假设如下:偏度和超额峰度都是 0。另一个假设如下:偏度或过度峰度不为零。
...
![img/500382_1_En_12_Fig48_HTML.jpg](img/500382_1_En_12_Fig48_HTML.jpg)
...
![$$ {D}_n=\underset{x}{\sup}\left|{F}_n(x)-F(x)\right| $$](img/500382_1_En_12_Chapter_TeX_Equhc.png)

其中![$$ \underset{x}{\sup } $$](img/500382_1_En_12_Chapter_TeX_IEq64.png)是经验分布函数*F*<sub>*n*</sub>(*x*)与累积分布函数 *F* ( *x* )之间的距离集合的上确界。直观地说，统计采用所有 *x* 值中两个分布函数之间的最大绝对差值。根据格利文科-坎特利定理，如果样本来自分布 *F* ( *x* ，那么 *D* <sub>* n *</sub> 在 *n* 趋于无穷大的极限内几乎必然收敛于 0。实际上，该统计需要相对大量的数据点(与其他拟合优度标准如 Anderson-Darling 检验统计相比),以正确拒绝零假设。如果 *F* ( *x* )的参数是根据数据确定的，例如 Lilliefors 测试中的样本均值和样本方差，则使用 Kolmogorov-Smirnov 分布确定的临界值无效。

双样本 Kolmogorov-Smirnov 检验评估两个基本的一维概率分布是否不同。两样本 KS 统计如下:

![$$ {D}_{n,m}=\underset{x}{\sup}\left|{F}_{1,n}(x)-{F}_{2,m}(x)\right| $$](img/500382_1_En_12_Chapter_TeX_Equhd.png)

其中 *F* <sub>1，*n*T5】(*x*)和 *F* <sub>2， *m*</sub> ( *x* )分别为第一个和第二个样本的经验分布函数，![$ \underset{x}{\sup } $](img/500382_1_En_12_Chapter_TeX_IEq65.png)为上确界函数。</sub>

双样本 KS 检验是比较两个样本的最有用和最通用的非参数方法之一，因为它对两个样本的经验累积分布函数的位置和形状的差异都很敏感。参见图 [12-49](#Fig49) 。

![img/500382_1_En_12_Fig49_HTML.png](img/500382_1_En_12_Fig49_HTML.png)
...
![$$ {\chi}^2=\sum \limits_{i=1}^r\sum \limits_{j=1}^c\frac{{\left({O}_{ij}-{E}_{ij}\right)}^2}{E_{ij}} $$](img/500382_1_En_12_Chapter_TeX_Equhe.png)
...
![$$ \Pr \left({X}_{n+1}=x\ |\ {X}_1={x}_1,{X}_2={x}_2,\dots, {X}_n={x}_n\right)=\Pr \left({X}_{n+1}=x\ |\ {X}_n={x}_n\right) $$](img/500382_1_En_12_Chapter_TeX_Equhf.png)

![img/500382_1_En_12_Fig50_HTML.png](img/500382_1_En_12_Fig50_HTML.png)
...
![$$ A=\left[\begin{array}{ccc}0.4&amp; 0.2&amp; 0.4\\ {}0.3&amp; 0.2&amp; 0.5\\ {}0.25&amp; 0.25&amp; 0.5\end{array}\right] $$](img/500382_1_En_12_Chapter_TeX_Equhg.png)
...
![$$ \Pr \left(Q\ |\ A\right)=\mathrm{P}\left({s}_1,{s}_1|{s}_1|{s}_3\ |\ A\right) $$](img/500382_1_En_12_Chapter_TeX_Equhh.png)

![$$ =\Pr \left({s}_3\ |\ A\right)\times \Pr \left({s}_1\ |{s}_3,A\right)\times \Pr \left({s}_1\ |{s}_1,{s}_3,A\right)\times \Pr \left({s}_1\ |{s}_1,{s}_1,{s}_3,A\right) $$](img/500382_1_En_12_Chapter_TeX_Equhi.png)

![$$ =\Pr \left({s}_3\ |\ A\right)\times \Pr \left({s}_1\ |{s}_3,A\right)\times \Pr \left({s}_1\ |{s}_1,A\right)\times \Pr \left({s}_1\ |{s}_1,A\right) $$](img/500382_1_En_12_Chapter_TeX_Equhj.png)

![$$ =1\times 0.25\times 0.4\times 0.4=0.04 $$](img/500382_1_En_12_Chapter_TeX_Equhk.png)

我们在步骤 3 中使用了马尔可夫属性来简化条件概率。具体地说，马尔科夫属性表示如下:

![$$ \Pr \left({q}_t|{q}_{t-1},\cdots, {q}_1\right)=\Pr \left({q}_t|{q}_{t-1}\ \right) $$](img/500382_1_En_12_Chapter_TeX_Equhl.png)
...
![img/500382_1_En_12_Fig51_HTML.png](img/500382_1_En_12_Fig51_HTML.png)

图 12-51

股票市场的隐马尔可夫模型

例如，扩展图 [12-51](#Fig51) 中的例子，市场可以处于三种状态:低、中和高波动性。股票在每个状态下的日收益率正态分布为零均值但波动率不同，![$$ {\sigma}_1^2\le {\sigma}_2^2\le {\sigma}_3^2 $$](img/500382_1_En_12_Chapter_TeX_IEq66.png)。请注意，每种状态都会产生相同的收益集(∞，+∞),但概率不同。我们不知道也无法准确判断市场在任何时间点都处于哪种状态。我们不仅不知道(隐藏的)状态，我们也不知道模型的参数， *λ* ，比如转移概率和![$$ \left\{{\sigma}_i^2\right\} $$](img/500382_1_En_12_Chapter_TeX_IEq67.png)。我们观察到的所有信息都是历史回报、观察序列ω(和 HMM 结构)。
...
![$$ \Pr \left(\Omega |\lambda\ \right)={\sum}_{{\left\{q\right\}}^{\prime }s}\Pr \left(\Omega, Q|\lambda \right)={\sum}_{{\left\{q\right\}}^{\prime }s}\Pr \left(\Omega |Q,\lambda \right)\times \Pr \left(Q|\lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equhm.png)

第一项是给定隐藏的状态序列 *Q* 时观察到ω的概率；第二项是给定 HMM 参数 *λ* 的隐藏状态序列的概率。对于第一个学期，我们有这样的:

![$$ \Pr \left(\Omega |Q,\lambda \right)={\prod}_{t=1}^T\Pr \left({\omega}_t|{q}_t,\lambda\ \right) $$](img/500382_1_En_12_Chapter_TeX_Equhn.png)
...
![$$ \Pr \left(Q|\lambda \right)={\pi}_{q_1}\times {a}_{q_1{q}_2}\times {a}_{q_2{q}_3}\times \cdots \times {a}_{q_{T-1}{q}_T} $$](img/500382_1_En_12_Chapter_TeX_Equho.png)

它只是初始状态概率![$$ {\pi}_{q_1} $$](img/500382_1_En_12_Chapter_TeX_IEq68.png)(从状态*q*T3】1 开始)和转移概率的乘积。

然而，这种天真的方法在计算上是不可行的，因为路径的数量呈指数增长。我们需要一种更有效的方法。

将前向概率*α*<sub>*t*</sub>(*I*)定义为部分观测序列直到时间 *t* 且系统在时间 *t* 处于状态 *s* <sub>*i*</sub> 的概率。也就是

![$$ {\alpha}_t(i)=\Pr \left({\omega}_1,{\omega}_2|\cdots |{\omega}_t|{q}_t={s}_i|\lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equhp.png)

这可以使用正向归纳来计算。初始化第一步，如下图:

![$$ {\alpha}_1(i)={\pi}_i{b}_i\left({\omega}_1\right) $$](img/500382_1_En_12_Chapter_TeX_Equhq.png)
...
![img/500382_1_En_12_Fig52_HTML.png](img/500382_1_En_12_Fig52_HTML.png)
...
![$$ {\alpha}_{t+1}(i)=\left[\sum \limits_{j=1}^N{\alpha}_t(i){a}_{ij}\right]{b}_i\left({\omega}_{t+1}\right) $$](img/500382_1_En_12_Chapter_TeX_Equhr.png)

其中 *N* 是 HMM 中状态的数量，*a*<sub>T5】ij</sub>是从状态 *i* 到状态 *j* 的转移概率。

最后，观察到ω给定 *λ* 的概率，简单来说就是跨越所有状态的所有 *α* <sub>*T*</sub> 之和。

![$$ \Pr \left(\Omega |\lambda\ \right)=\sum \limits_{j=1}^N{\alpha}_T(j) $$](img/500382_1_En_12_Chapter_TeX_Equhs.png)
...
![img/500382_1_En_12_Fig53_HTML.jpg](img/500382_1_En_12_Fig53_HTML.jpg)
...
![$$ {\beta}_t(i)=\Pr \left({\omega}_{t+1},{\omega}_{t+2}|\cdots, {\omega}_T,\lambda, {q}_t={s}_i\right) $$](img/500382_1_En_12_Chapter_TeX_Equht.png)

这可以使用逆向归纳法来计算。初始化最后一步，如图 [12-54](#Fig54) 所示。【T2![$$ {\beta}_T(i)=1 $$](img/500382_1_En_12_Chapter_TeX_Equhu.png)

![img/500382_1_En_12_Fig54_HTML.png](img/500382_1_En_12_Fig54_HTML.png)
...
![$$ {\beta}_t(i)=\sum \limits_{j=1}^N{a}_{ij}{b}_j\left({\omega}_{t+1}\right){\beta}_{t+1}(j) $$](img/500382_1_En_12_Chapter_TeX_Equhv.png)

最后，给定观测值ω和参数 *λ* ，系统在时刻 *t* 处于状态 *i* 的概率如下:

![$$ {\gamma}_t(i)=\Pr \left({q}_t={s}_i\ |\ \Omega, \lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equhw.png)

![$$ =\frac{\Pr \left({q}_t={s}_i,\Omega |\lambda \right)}{\Pr \left(\Omega |\lambda \right)} $$](img/500382_1_En_12_Chapter_TeX_Equhx.png)

![$$ =\frac{\alpha_t(i){\beta}_t(i)}{\Pr \left(\Omega |\lambda \right)} $$](img/500382_1_En_12_Chapter_TeX_Equhy.png)

![$$ =\frac{\alpha_t(i){\beta}_t(i)}{\sum \limits_{j=1}^N{\alpha}_t(j){\beta}_t(j)} $$](img/500382_1_En_12_Chapter_TeX_Equhz.png)

在不考虑状态序列出现概率的情况下，每一瞬间最可能的状态 *t* 如下:

![$$ {q}_t=\underset{1\le i\le N}{\mathrm{argmax}}\left[{\gamma}_t(i)\right] $$](img/500382_1_En_12_Chapter_TeX_Equia.png)

最可能的隐藏状态序列，即在所有可能的状态序列中找到最大值，可以使用维特比算法来求解。定义系统行进这些状态的最大概率，在时间 *t* 时着陆状态 *s* <sub>*i*</sub> ，并生成观测值:

![$$ {\delta}_t(i)=\max \left[\Pr \left({q}_1,{q}_2|\cdots |{q}_t={s}_i|{\omega}_0|\cdots |{\omega}_t|\lambda \right)\right] $$](img/500382_1_En_12_Chapter_TeX_Equib.png)

初始化:

![$$ {\delta}_1(i)={\pi}_i{b}_i\left({\omega}_1\right) $$](img/500382_1_En_12_Chapter_TeX_Equic.png)

T3】![$$ {\psi}_1=0 $$](img/500382_1_En_12_Chapter_TeX_Equid.png)T5】

![$$ {\delta}_t(j)=\underset{i}{\max}\left[{\delta}_{t-1}(i){a}_{ij}{b}_j\left({\omega}_t\right)\right] $$](img/500382_1_En_12_Chapter_TeX_Equie.png)

递归

这里的*δ*<sub>*t*</sub>(*I*)是最可能状态序列的概率 *q* <sub>1</sub> ， *q* <sub>2</sub> ， *q* <sub>*t*</sub> 负责第一个 *t* 的观测，以 *j* 为其最终状态

![$$ {\psi}_t=\underset{i}{\mathrm{argmax}}\left[{\delta}_{t-1}(i){a}_{ij}\right] $$](img/500382_1_En_12_Chapter_TeX_Equif.png)

其中*ψ*<sub>T3】tT5 是在时间 *t* 选择的状态，以最大化*δ*<sub>*t*</sub>(*j*)。</sub>

终止:

![$$ {\delta_T}^{\ast }=\underset{i}{\max }{\delta}_T(i) $$](img/500382_1_En_12_Chapter_TeX_Equig.png)

![$$ {q}_T=\underset{i}{\mathrm{argmax}}{\delta}_T $$](img/500382_1_En_12_Chapter_TeX_Equih.png)
...
![img/500382_1_En_12_Fig55_HTML.jpg](img/500382_1_En_12_Fig55_HTML.jpg)
...
*![$$ {\xi}_t\left(i,j\right)=\Pr \left({q}_t={s}_i,{q}_{t+1}={s}_j\ |\ \Omega, \lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equii.png)*

我们可以用向前和向后的概率来写。

![$$ {\xi}_t\left(i,j\right)=\Pr \left({q}_t={s}_i,{q}_{t+1}={s}_j\ |\ \Omega, \lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equij.png)

![$$ =\frac{\Pr \left({q}_t={s}_i,{q}_{t+1}={s}_j|\Omega |\lambda \right)}{\Pr \left(\Omega |\lambda \right)} $$](img/500382_1_En_12_Chapter_TeX_Equik.png)

![$$ =\frac{\alpha_t(i){a}_{ij}{b}_j\left({\omega}_{t+1}\right){\beta}_{t+1}(j)}{\Pr \left(\Omega |\lambda \right)} $$](img/500382_1_En_12_Chapter_TeX_Equil.png)

因此，我们有了这个:

![$$ {\gamma}_t(i)=\Pr \left({q}_t={s}_i\ |\ \Omega, \lambda \right) $$](img/500382_1_En_12_Chapter_TeX_Equim.png)

![$$ =\sum \limits_{j=1}^N{\xi}_t\left(i,j\right) $$](img/500382_1_En_12_Chapter_TeX_Equin.png)
...
![$$ \hat{\pi_i}={\gamma}_1(i) $$](img/500382_1_En_12_Chapter_TeX_IEq69.png)，初始状态概率

![$$ \hat{a_{ij}}=\frac{\sum \limits_{t=1}^{T-1}{\xi}_t\left(i,j\right)}{\sum \limits_{t=1}^{T-1}{\gamma}_t(i)} $$](img/500382_1_En_12_Chapter_TeX_IEq70.png)，转移概率

![$$ \hat{b_{j\left({v}_k\right)}}=\frac{\sum \limits_{t=1,{\omega}_t={v}_k}^{T-1}{\gamma}_t(j)}{\sum \limits_{t=1}^{T-1}{\gamma}_t(i)} $$](img/500382_1_En_12_Chapter_TeX_IEq71.png)，状态 *s* <sub>* j *</sub> 下观察值的条件概率，用于离散观察值
...
![img/500382_1_En_12_Fig56_HTML.png](img/500382_1_En_12_Fig56_HTML.png)
...
![img/500382_1_En_12_Fig57_HTML.png](img/500382_1_En_12_Fig57_HTML.png)
...
![$$ \boldsymbol{X}=\left[\begin{array}{c}{x}_A\\ {}{y}_A\\ {}{x}_B\\ {}{y}_B\\ {}{x}_C\\ {}{y}_C\end{array}\right] $$](img/500382_1_En_12_Chapter_TeX_Equio.png)

![img/500382_1_En_12_Fig58_HTML.jpg](img/500382_1_En_12_Fig58_HTML.jpg)
...
![$$ \boldsymbol{PX}=\boldsymbol{Y} $$](img/500382_1_En_12_Chapter_TeX_Equip.png)
...
![$$ {\boldsymbol{S}}_{\boldsymbol{Y}}=\frac{1}{n-1}\boldsymbol{Y}{\boldsymbol{Y}}^T $$](img/500382_1_En_12_Chapter_TeX_Equiq.png)

![$$ =\frac{1}{n-1}\left(\boldsymbol{PX}\right){\left(\boldsymbol{PX}\right)}^T $$](img/500382_1_En_12_Chapter_TeX_Equir.png)

![$$ =\frac{1}{n-1}\boldsymbol{PX}{\boldsymbol{X}}^T\boldsymbol{P} $$](img/500382_1_En_12_Chapter_TeX_Equis.png)

注意***A***=***XX***<sup>*T*</sup>是***×15***的协方差矩阵(去除样本均值和比例因子后)。它是对称的，因此是可对角化的。存在特征向量矩阵 ***E*** 和对角矩阵 ***D*** 使得:

![$$ \boldsymbol{A}=\boldsymbol{ED}{\boldsymbol{E}}^T $$](img/500382_1_En_12_Chapter_TeX_Equit.png)

假设我们选择基的变化***P = E***<sup>*T*</sup>；我们有以下:

![$$ {\boldsymbol{S}}_{\boldsymbol{Y}}=\frac{1}{n-1}\boldsymbol{P}{\boldsymbol{P}}^T\boldsymbol{D}{\boldsymbol{P}}^T\boldsymbol{P} $$](img/500382_1_En_12_Chapter_TeX_Equiu.png)

![$$ =\frac{1}{n-1}\boldsymbol{P}{\boldsymbol{P}}^{-1}\boldsymbol{D}{\boldsymbol{P}}^{-1}\boldsymbol{P} $$](img/500382_1_En_12_Chapter_TeX_IEq72.png)，因为 *** P *** 是正交的

![$$ =\frac{1}{n-1}\boldsymbol{D} $$](img/500382_1_En_12_Chapter_TeX_Equiv.png)
...
**![$$ \boldsymbol{X}=\boldsymbol{U}\boldsymbol{\Sigma } {\boldsymbol{V}}^T $$](img/500382_1_En_12_Chapter_TeX_Equiw.png)**

 **相当于，

![$$ {\boldsymbol{U}}^T\boldsymbol{X}=\boldsymbol{\Sigma} {\boldsymbol{V}}^T $$](img/500382_1_En_12_Chapter_TeX_Equix.png)

在这里，我们又有了如下的变化依据:

![$$ \boldsymbol{P}={\boldsymbol{U}}^T $$](img/500382_1_En_12_Chapter_TeX_Equiy.png)

而新的坐标为:

![$$ \boldsymbol{Y}=\boldsymbol{\Sigma} {\boldsymbol{V}}^T $$](img/500382_1_En_12_Chapter_TeX_Equiz.png)
...
![$$ {\displaystyle \begin{array}{c}{z}_{1,i}={l}_{1,1}{F}_{1,i}+{l}_{1,2}{F}_{2,i}+{\varepsilon}_{1,i}\\ {}\vdots \\ {}{z}_{10,i}={l}_{10,1}{F}_{1,i}+{l}_{10,2}{F}_{2,i}+{\varepsilon}_{10,i}\end{array}} $$](img/500382_1_En_12_Chapter_TeX_Equja.png)

或者，更简洁地说，可以这样写:

![$$ {z}_{a,i}=\sum \limits_{p=1}^2{l}_{a,1}{F}_{p,i}+{\varepsilon}_{a,i} $$](img/500382_1_En_12_Chapter_TeX_Equjb.png)

其中 *F* <sub>1，*I*T5】为学生 *i* 的言语智力， *F* <sub>2， *i*</sub> 为学生 *i* 的数学智力， *l* <sub>*a* ， *p*</sub> 为*a*-的因子载荷见图 [12-59](#Fig59) 。</sub>

![img/500382_1_En_12_Fig59_HTML.png](img/500382_1_En_12_Fig59_HTML.png)
...
![$$ Z= LF+\varepsilon $$](img/500382_1_En_12_Chapter_TeX_Equjc.png)
...
![$$ \frac{3000^2+3000}{2}=4,504,500 $$](img/500382_1_En_12_Chapter_TeX_Equjd.png)
...
![$$ \underset{\boldsymbol{X}}{\max}\left\{\log \det \left(\boldsymbol{X}\right)- Tr\left(\boldsymbol{\Sigma} \boldsymbol{X}\right)-\rho \mathrm{Card}\left(\boldsymbol{X}\right)\right\} $$](img/500382_1_En_12_Chapter_TeX_Equje.png)
...
![$$ \underset{\boldsymbol{X}}{\max}\left(\log \det \left(\boldsymbol{X}\right)- Tr\left(\boldsymbol{\Sigma} \boldsymbol{X}\right)-\rho \sum \limits_{i,j=1}^n\left|{X}_{ij}\right|\right) $$](img/500382_1_En_12_Chapter_TeX_Equjf.png)

涉及 ***X*** 的条目的绝对值之和的惩罚项充当基数的代理:函数![$$ \sum \limits_{i,j=1}^n\left|{X}_{ij}\right| $$](img/500382_1_En_12_Chapter_TeX_IEq73.png)可以被视为超立方体上用于秩最小化的 Card( *** X *** )上的最大凸下界。它也常用于回归和变量选择程序，如套索。
...
![img/500382_1_En_12_Fig60_HTML.jpg](img/500382_1_En_12_Fig60_HTML.jpg)
...
![img/500382_1_En_12_Fig61_HTML.jpg](img/500382_1_En_12_Fig61_HTML.jpg)
...
![img/500382_1_En_12_Fig62_HTML.jpg](img/500382_1_En_12_Fig62_HTML.jpg)
