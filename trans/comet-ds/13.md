...
你需要按照第一章 、*Comet概述*中的说明配置`.comet.config`文件。或者，要将附加参数传递给 Comet 实验，您可以将它们包含在`.comet.config`文件中。
...
现在你已经准备好构建最终报告了。在这个例子中，我们用模型结果构建了一个简单的报告。作为进一步的练习，你可以通过应用在 [*第五章*](B17530_05_ePub.xhtml#_idTextAnchor105) ，*在Comet*中建立一个叙述的概念来改进它们。
...
2.  然后，我们插入记录的样本图像。我们使用的是Comet查看器面板，你可以通过点击**添加** | **新建面板** | **公共** | **Comet查看器面板** | **添加** | **完成**来添加。
