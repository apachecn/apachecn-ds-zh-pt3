...
我们已经在 [*第一章*](B17530_01_ePub.xhtml#_idTextAnchor015) 、*Comet概述*中描述了前两个包以及如何安装它们，所以关于安装的更多细节请参考后面的内容。
...
这意味着您不仅要监控预测服务，还必须确保模型表现良好，不会随着时间的推移而退化。如果模型随时间退化，您必须实现一种自动技术来更新模型和使用它的预测服务。这就是Comet出现的地方。
...
这就是Comet帮助你的地方。Comet 提供了工具来跟踪您的代码、实验等中的所有变更，以便轻松地与部署阶段集成。下图显示了如何将 Comet 与部署阶段集成在一起:
...
作为用例，我们使用在 [*第三章*](B17530_03_ePub.xhtml#_idTextAnchor063)*中实现的示例的简化版本，在Comet*中进行模型评估。目标是构建一个独立的应用程序，测试四种不同的分类模型(随机森林、决策树、高斯朴素贝叶斯和 k-最近邻)，根据一些输入参数将切割钻石分为两类(黄金和白银)。此外，应用程序在 Comet 中记录所有的模型。作为辅助函数，我们使用第三章 、*中描述的那些在Comet*中的模型评估。
...
1.  现在，我们在文件内复制第三章[](B17530_03_ePub.xhtml#_idTextAnchor063)**中描述的所有函数，在Comet*中进行模型评估。我们只修改`run_experiment()`函数，让 Comet 读取环境变量:
...
我们已经在 [*第三章*](B17530_03_ePub.xhtml#_idTextAnchor063) 中描述了代码，在Comet中*模型评估。唯一不同的是，我们添加了行`if __name__ == "__main__":`来运行代码*，如果它被直接调用的话。**
