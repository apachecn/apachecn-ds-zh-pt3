<title>Chapter 7: Extending the GitLab DevOps Platform with Comet</title> <link href="css/style-JRserifv5.css" rel="stylesheet" type="text/css">

# *第七章*:用 Comet 扩展 GitLab DevOps 平台
...
我们在 [*第一章*](B17530_01_ePub.xhtml#_idTextAnchor015) 、*慧星概述*中描述了前两个包以及如何安装。有关安装的更多详细信息，请参阅后面的内容。
...
SCS由一个中央服务器存储库组成，该存储库存储代码，并跟踪其所有历史，如下所示:
...
*图 7.5* 显示git lab 平台提出创建分支的合并请求(图顶部绿色矩形，带蓝色按钮)。以下是创建合并请求的步骤:
...
一旦您配置了Comet web hook，每当您的模型的阶段发生变化时，就会向您的外部服务发送一个通知。
