...
分层数据通常比平面数据更好地表示具有属性和互相关性的实体。在面向对象编程中，但也仅仅是在普通分类法和[本体论](Glossary.xhtml#_idTextAnchor090)中，关系*是-a* 和*有-a* 通常是基本的，而且这两者都不是表格。或者说充其量就算是*捕捉到的属性也有-a* 关系，参差不齐，而且肯定不整齐。此外， *is-a* 关系的核心是层次化的。
