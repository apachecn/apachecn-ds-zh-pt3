...
我最终选定了Pandas和功能，因为这是一本关于Pandas的书，并且与主题保持一致。此外，函数更容易阅读和使用。Pandas可能不是扩大生产规模的最佳选择。
...
这段代码将所有的字典键转换成一个列表，这样我们就可以用它作为索引来构建一个Pandas数据框架。
...
再往下的`instantiate_portfolio`函数，我们称之为`.from_` `dict`。这段代码简单地从字典中生成了一个Pandas数据帧。
...
`simulate_process_order`函数接受投资组合对象和Pandas系列对象来买卖证券。
