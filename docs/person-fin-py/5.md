...
在我们定义了开始和结束日期之后，我们可以创建一个包含Pandas的空日历对象，它利用了由`date_range`函数创建的`DatetimeIndex`。
...
*   在`pd.concat`中`axis`被设置为`1`来告诉Pandas我们想要沿着列的轴缝合(连接)。
...
现金流不规则。我们可能会在特定的日子里支出费用，或者按照越来越奇怪的时间表获得报酬。用原始英语定义这些频率会更好，而不是必须依赖Pandas偏移别名。
...
要将这些日期包装到 DataFrame 对象中，我们必须将列表转换为 DatetimeIndex 对象。唯一的问题是`pd.DatetimeIndex`只接受一只Pandas`Series`。但其实没问题！
